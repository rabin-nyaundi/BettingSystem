defmodule BettingsystemWeb.UserAccountsForgotPasswordLive do
  ...
  def handle_event("send_email", %{"user_accounts" => %{"email" => email}}, socket) do
    if user_accounts = Account.get_user_accounts_by_email(email) do
      Account.deliver_user_accounts_reset_password_instructions(
        user_accounts,
        &url(~p"/user_acconts/reset_password/#{&1}")
      )
    end
